...
def apkGenerator(yaml_path, aId):
    ...
    working_dir = "/d/Bhargo/Airflow/configuration"
    ...
    subprocess.call(clone_command.split(), shell=True)
    ...
    with open(working_dir+file_path+file_name, "w") as file:
    ...
    subprocess.call(f"copy {working_dir}\\local.properties {working_dir}\\{repo_name}\\", shell=True)
        
    subprocess.call(f"copy {working_dir}\\gradle-wrapper.jar {working_dir}\\{repo_name}\\gradle\\wrapper\\", shell= True)

    # Build Gradle
    
    # subprocess.call(f"{working_dir}\\{repo_name}\\.\\gradlew.bat clean assembleDebug", cwd=f"{working_dir}\\{repo_name}\\", shell=True)
    #
